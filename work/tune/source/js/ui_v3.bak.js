...
var alertOpen = function(){ 
  ...
  //   $('#'+$(this).data("popname")+'').addClass('alartOn'); 
  //   $("html").css("overflow", "hidden"); }); 
  //   $('.alartClose, .alartDim').click(function(){ 
  //     $(this).parents('.alartWrap').removeClass('alartOn'); 
  //     $("html").css("overflow", "auto"); 
  //   }); 
  };
// var alertOpen = function(){
...
//     // 클릭 위치 판단 (요소의 중앙 좌표 기준)
//     const offset = $this.offset().left + $this.outerWidth() / 2;
//     const winCenter = $(window).width() / 2;

//     // 기본 클래스 제거
//     $wrap.removeClass('pos-left pos-right');

//     // 위치 판단 후 클래스 부여
//     if (offset > winCenter) {
//       // 화면 중앙보다 오른쪽 → wrap을 왼쪽에 띄움
//       $wrap.addClass('pos-left');
//     } else {
//       // 화면 중앙보다 왼쪽 → wrap을 오른쪽에 띄움
//       $wrap.addClass('pos-right');
//     }

//     // 팝업 열기
//     $wrap.addClass('alertOn');
//     $("html").css("overflow", "hidden"); 
//   }); 

//   // 닫기
//   $('.alertClose, .alertDim').click(function(){
//     $(this).parents('.alertWrap').removeClass('alertOn pos-left pos-right');  
//     $("html").css("overflow", "auto");
//   }); 
// };

var alertOpen2 = function(){
  var iMarkBtn = $(".bubbleOpen");

  // iMarkBtn.each(function () {
  //   $(this).on('mouseover', function () {
  //     let $wrap = $(this).find(".bubbleWrap"); // 1️⃣ 자식 bubbleWrap 찾기

  //     if ($wrap.length === 0) {
  //       $wrap = $(this).siblings(".bubbleWrap").first(); // 2️⃣ 없으면 형제 중 가장 가까운 bubbleWrap
  //     }
  ...
  //     let $wrap = $(this).find(".bubbleWrap");

  //     if ($wrap.length === 0) {
  //       $wrap = $(this).siblings(".bubbleWrap").first();
  //     }
  ...
  $(".bubbleOpen").hover(
    function () {
      const $open = $(this);
      const $wrap = $open.find(".bubbleWrap");

      if ($wrap.length) {
        // 원래 부모 위치 기억
        $open.data("orig-parent", $wrap.parent());

        // .container 바로 아래로 이동
        $(".container").prepend($wrap);

        // 버튼 위치 기준으로 배치
        const offset = $open.offset();
        $wrap.css({
          position: "absolute",
          top: offset.top + $open.outerHeight(),
          left: offset.left,
          display: "block",
          // zIndex: 900
        });
    ...
    function () {
      const $open = $(this);
      const $wrap = $(".wrap > .bubbleWrap"); // 이동된 wrap 찾기
      const $origParent = $open.data("orig-parent");

      if ($wrap.length && $origParent) {
        $wrap.hide().appendTo($origParent); // 다시 원래 자리로
      }
    }
  );
  $('.bubbleClose, .modalDim').click(function(){
    $(this).parents('.bubbleWrap').removeClass('modalOn');  
    $("html").css("overflow", "auto");
  }); 
};
...
var bubbleOpen_bak = function () {
  $('.bubbleOpen').click(function () {
    const popName = $(this).data("popname"); // data-popname 값 (예: "bw_userStats")
    const $wrap = $('.bubbleWrap.' + popName); // 동일 클래스 찾기
  ...
  // 닫기 버튼 / 딤 처리
  $('.bubbleClose, .alartDim').click(function () {
    $(this).parents('.bubbleWrap').removeClass("bubbleOn").hide();
    $("html").css("overflow", "auto");
  });
};

// bubbleOpen();
var bubbleOpen_bak2 = function () {
  $('.bubbleOpen').click(function () {
    const $btn = $(this);
    const popName = $btn.data("popname"); // data-popname (예: bw_userStats)
    const $wrap = $('.bubbleWrap.' + popName);

    if ($wrap.length) {
      const winHeight = window.innerHeight; // 화면 세로 크기
      const btnOffset = $btn.offset();
      const btnTop = btnOffset.top;
      const btnBottom = winHeight - (btnTop + $btn.outerHeight());

      // 기본 세팅
      $wrap.removeClass("pos-top pos-bottom").hide();

      //위/아래 방향 결정 (화면 중앙 기준)
      // if (btnTop > winHeight / 2) {
      //   // 버튼이 화면 위쪽 → bubbleWrap은 위로 띄움
      //   $wrap.addClass("pos-top");
      //   $(".wrap").prepend($wrap);
      // } else {
      //   // 버튼이 화면 아래쪽 → bubbleWrap은 아래로 띄움
      //   $wrap.addClass("pos-bottom");
      //           $(".wrap").prepend($wrap);

      // }

      // 윈도우 스크롤 고려해서 실제 화면상의 위치로 비교
...
// 버튼 위/아래 남는 공간 비교해서 여유있는 쪽으로 나타나게 
//       if (btnBottom > btnTop) {
//   // 아래쪽 공간이 더 넓으면 아래에 띄움
//   $wrap.addClass("pos-bottom");
//   $(".wrap").prepend($wrap);
// } else {
//   // 위쪽 공간이 더 넓으면 위에 띄움
...
// }
  ...
  // 닫기
  $('.bubbleClose, .bubbleDim').click(function () {
    $(this).parents('.bubbleWrap').removeClass("bubbleOn pos-top pos-bottom").hide();
  });
};

var bubbleOpen22 = function () {
  $('.bubbleOpen').click(function (e) {
    e.stopPropagation();

    const $btn = $(this);
    const popName = $btn.data("popname");
    const $wrap = $('.bubbleWrap.' + popName);
...
var bubbleOpen3 = function () {
  $('.bubbleOpen').click(function (e) {
    e.stopPropagation();

    const $btn = $(this);
    const popName = $btn.data("popname");
    // ✅ 클릭한 bubbleOpen 내부에서만 bubbleWrap 찾기
    const $wrap = $btn.find('.bubbleWrap.' + popName);
...
var bubbleOpen6 = function () {
  $(document).on("click", ".bubbleOpen", function (e) {
    e.stopPropagation();

    const $btn = $(this);
    const $wrap = $btn.find(".bubbleWrap"); // 자기 안에 있는 것만 찾음
...
var bubbleOpen7 = function () {
  $(document).on("click", ".bubbleOpen", function (e) {
    e.stopPropagation();

    const $btn = $(this);
    const bubbleName = $btn.data("bubbleName") || $btn.data("boName"); // 버튼에 있는 식별자
    const $wrap = $('.bubbleWrap[data-bubbleName="' + bubbleName + '"]');
    ...
    if ($wrap.length) {
      ...
      // wrap을 wrap-container(.wrap) 아래로 이동
      $(".wrap").prepend($wrap);
...
// bubbleOpen();
//* click 막기 */
var cdp_dashboard222 = function () {
  var bubbleOpenSelector = '.tune_cdpDashboard .sec_analysis2 .chartArea .box .bg .bubbleOpen';
  var bgSelector = '.tune_cdpDashboard .sec_analysis2 .chartArea .box .bg';
...
var tune_aiSlide = function() {
  ...
  var slideContainer2 = $('.tune_aiQuestion .box_question .slick, .tune_aiAnswer .sec_question .slick');
  var slideContainer3 = $('.tune_aiAnswer .sec_answer .slick');
...
var tune_aiHome = function () {
  var aiHome = $(".container .tune_aiHome");
  var aiAnswer = $(".container .tune_aiAnswer.t2");
  
  var tune_ai = $(".container [class^='tune_ai']");

  // tune_aiHome 클래스가 있으면 html 태그에 스크롤 제외
  if ($(aiHome).length > 0) {
    $('html').css('overflow-y', 'hidden');
    aiHome.closest('.container').addClass("tune_aiHome");
  }

    // aiAnswer 클래스가 있으면 html 태그에 스크롤 제외
...
//   }
  ...
  // t1 > t2로 바꾸는 테스트 js
  $('.tune_aiHome .box_customQuestion a').on('click', function(e) {
    e.preventDefault(); // 링크 기본 동작 방지 (필요시)
  
    const $parent = $(this).closest('.tune_aiHome');
    
    if ($parent.hasClass('t1')) {
      $parent.removeClass('t1').addClass('t2');
    }
  });

  //* tune_aiHome 화면만 제어 */
  // tune_aiHome 화면에서 bookmark 클릭시 on 클래스 토글
  var toggle = $(".tune_aiHome").find("button.bookmark, button.voice, button.sound");
 ...
 // 동동이 박스옆에 따라다니는 스크립트
    ...
    const $qnaBox = $('.tune_aiHome .qnaBox, .tune_aiAnswer .qnaBox');
...
$(function () {
  ...
  tune_aiHome(); // AI스튜디오 메뉴 > html태그 스크롤 제거
